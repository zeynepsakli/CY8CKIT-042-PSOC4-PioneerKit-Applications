...
#include "LED_Red.h"
    ...
    #define LED_Red_SetP4PinDriveMode(shift, mode)  \
    do { \
        LED_Red_PC =   (LED_Red_PC & \
                                (uint32)(~(uint32)(LED_Red_DRIVE_MODE_IND_MASK << \
                                (LED_Red_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (LED_Red_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define LED_Red_SetP4PinDriveMode(shift, mode)  \
        do { \
            LED_Red_USBIO_CTRL_REG = (LED_Red_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(LED_Red_DRIVE_MODE_IND_MASK << \
                                    (LED_Red_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (LED_Red_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		LED_Red_SetP4PinDriveMode(LED_Red__0__SHIFT, mode);
    }
#endif
...
void LED_Red_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  LED_Red_INTCFG & (uint32)(~(uint32)position);
    LED_Red_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
