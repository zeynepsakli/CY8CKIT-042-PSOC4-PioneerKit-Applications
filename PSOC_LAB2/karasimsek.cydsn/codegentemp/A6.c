...
#include "A6.h"
...
#if defined(A6__PC)
    #define A6_SetP4PinDriveMode(shift, mode)  \
    do { \
        A6_PC =   (A6_PC & \
                                (uint32)(~(uint32)(A6_DRIVE_MODE_IND_MASK << \
                                (A6_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (A6_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define A6_SetP4PinDriveMode(shift, mode)  \
        do { \
            A6_USBIO_CTRL_REG = (A6_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(A6_DRIVE_MODE_IND_MASK << \
                                    (A6_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (A6_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		A6_SetP4PinDriveMode(A6__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(A6_DR & (uint8)(~A6_MASK));
    drVal = (drVal | ((uint8)(value << A6_SHIFT) & A6_MASK));
    A6_DR = (uint32)drVal;
}
...
uint8 A6_Read(void)
{
    return (uint8)((A6_PS & A6_MASK) >> A6_SHIFT);
}
...
{
    return (uint8)((A6_DR & A6_MASK) >> A6_SHIFT);
}
...
void A6_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  A6_INTCFG & (uint32)(~(uint32)position);
    A6_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(A6_INTSTAT & A6_MASK);
	A6_INTSTAT = maskedStatus;
    return maskedStatus >> A6_SHIFT;
}
